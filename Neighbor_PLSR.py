...
for t, neighbor_dict in enumerate(neighbor_list):
    ...
    for i in range(len(sensor_id_list_la)):
        ...
        for k in range(1, len(neighbor_dict[i])):
            ...
            ### Training PLSR with top-5 Neighbors ###
            regr_wo = make_pipeline(StandardScaler(), PLSRegression(n_components=20))
...
result_df.to_csv("../Results/Neighbor_PLSR_performance_table.csv")
